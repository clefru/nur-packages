...
let ibDerivation = stdenv.mkDerivation rec {
  ...
  installPhase = ''
    ...
    mkdir $out/bin
    sed -e s#__OUT__#$out# -e s#__JAVAHOME__#${pkgs.openjdk8.jre}/lib/openjdk/jre# -e s#__GTK__#${pkgs.gtk3}# -e s#__CCLIBS__#${pkgs.stdenv.cc.cc.lib}# ${./tws-wrap.sh} > $out/bin/ib-tws-native
